...
declare namespace App {
  ...
  namespace I18n {
    ...
    /** Route keys available in `route.*` i18n (incl. reserved business routes with copy but no page yet). */
    type I18nRouteKey =
    ...
    type Schema = {
      ...
      /** Shared list/table field labels (API keys → display). */
      field: {
        id: string;
        ip: string;
        scope: string;
      ...
      /** Runtime summary phase codes from provider → platform. */
      phase: {
        idle: string;
        preparing: string;
        in_round: string;
