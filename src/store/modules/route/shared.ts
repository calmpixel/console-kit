...
function getGlobalMenuByBaseRoute(route: RouteLocationNormalizedLoaded | ElegantConstRoute) {
  ...
  // 动态菜单（后端）：title 优先；仅当 title 为空时才用 i18nKey（静态内置路由）
  const label = resolveRouteLabel(title, i18nKey);

  const menu: App.Global.Menu = {
    key: name as string,
    label,
    // 有 title 时不再挂 i18nKey，切语言也不会把运营标题刷掉
    i18nKey: title?.trim() ? undefined : i18nKey,
